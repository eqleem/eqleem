...
<template>
    <div class="min-w-0 divide-y divide-dotted divide-stone-200">
        <div class="flex w-full min-w-0 items-center gap-x-3 bg-white p-3 sm:gap-x-7">
            ...
            <div class="min-w-0 flex-1 rounded-lg bg-stone-100">
        ...
        <div class="relative min-w-0 overflow-visible p-1">
            ...
            >
                ...
                >
                    ...
                    <div class="flex min-w-0 flex-1 items-center gap-x-2">
                        <div class="h-10 w-10 shrink-0 rounded-full bg-stone-200"></div>
                        <div class="min-w-0 flex-1 space-y-2">
                            <div
                                class="h-4 max-w-full rounded-md bg-stone-200 sm:h-5"
                                :class="n % 3 === 0 ? 'w-2/5 sm:w-28' : n % 2 === 0 ? 'w-1/2 sm:w-40' : 'w-2/5 sm:w-36'"
                            ></div>
                            <div class="flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1">
                                <div class="h-2 w-2 shrink-0 rounded-full bg-stone-200"></div>
                                <div
                                    class="h-4 max-w-[55%] rounded-md bg-stone-100 sm:h-5 sm:max-w-none"
                                    :class="n % 2 === 0 ? 'w-24 sm:w-32' : 'w-28 sm:w-40'"
                                ></div>
                                <div class="hidden h-4 w-16 rounded-md bg-stone-100 sm:block sm:w-20"></div>
                            </div>
                        </div>
                    </div>
            ...
            <div
                ...
                class="flex flex-col items-center justify-center gap-2 p-10 text-center"
            >
                <svg class="h-12 w-12 p-0.5 text-stone-400" viewBox="0 0 24 24">
                ...
                <p class="text-stone-700">لا يوجد عملاء.</p>
                <small class="text-stone-500">سيتم عرض العملاء هنا بعد إضافتهم أو شراء أحد المنتجات أو الخدمات.</small>
            </div>
            ...
            <div v-else-if="items.length > 0" class="divide-y-2 divide-dotted divide-stone-200/50">
                <div
                    v-for="item in items"
                    :key="item.id"
                    class="flex w-full items-center justify-between gap-x-7 last:rounded-b-2xl hover:bg-stone-50"
                >
                    <div class="ps-6">
                        <div class="flex items-center">
                    ...
                    <div class="w-full truncate py-3">
                        <RouterLink :to="`/clients/${item.uuid}`" class="flex items-center gap-x-2">
                            <img class="h-10 w-10 flex-none rounded-full bg-stone-50" :src="item.avatar" alt="">
                            <div>
                                <h2 class="text-lg text-stone-700">{{ item.name }}</h2>
                                <div class="mt-1 flex items-center gap-x-2">
                                    <div class="flex items-center gap-x-2 text-base leading-6 text-stone-900">
                                    ...
                                    <p class="mt-1 flex items-center gap-1 truncate text-xs text-stone-500">
                                        <span
                                            v-if="item.email"
                                            class="inline-flex items-center gap-x-1 truncate rounded-md bg-stone-100 p-1 px-2 text-xs"
                                        >{{ item.email }}</span>
