    ...
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-7 bg-white p-3">
            <div class="flex-grow bg-stone-100 rounded-lg">
                <div class="relative col-span-3 text-sm text-stone-800">
                    ...
                    <input v-model="searchInput" type="text" placeholder="ابحث برقم الفاتورة أو الطلب .." class="block w-full rounded-lg border border-transparent py-1.5 ps-10 text-stone-800 ring-inset ring-stone-200 placeholder:text-stone-400 focus:border-primary-500 focus:outline-none sm:text-sm sm:leading-6">
                </div>
            </div>
        </div>
        ...
        <div class="relative p-1">
                ...
                >
                    <div class="min-w-0 flex-1 space-y-2 py-3">
                        ...
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                            ...
                            ></div>
                            <div class="h-3 w-20 rounded-md bg-stone-100"></div>
                        </div>
                    </div>
                    ...
                    <div class="hidden shrink-0 items-center gap-x-6 sm:flex">
                        <div class="space-y-1.5 text-end">
                            ...
                            <div class="ms-auto h-3 w-16 rounded-md bg-stone-100"></div>
                        </div>
            ...
            <div v-else-if="isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                <svg class="h-12 w-12 p-0.5 text-stone-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6 3h9l3 3v15H6zM14 3v4h4M9 12h6M9 16h6" /></svg>
                <p class="text-stone-700">لا توجد فواتير.</p>
                <small class="text-stone-500">ستظهر فواتير المتجر هنا بعد تسجيل أي دفعة أو إصدار فاتورة.</small>
            </div>
                ...
                <div v-for="item in items" :key="item.id" class="flex w-full items-center justify-between gap-x-4 px-4 last:rounded-b-2xl hover:bg-stone-50 sm:px-6">
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/invoices/${item.uuid}`" class="block">
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                                <h2 class="text-lg font-semibold text-stone-700" dir="ltr">{{ item.s_number }}</h2>
                            ...
                            <div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-stone-500">
                                <span v-if="item.order_label" class="inline-flex items-center gap-x-1 rounded-md bg-stone-100 p-1 px-2 text-xs">{{ item.order_label }}</span>
                                <span v-if="item.issued" class="text-xs text-stone-400">{{ item.issued }}</span>
                            </div>
                        </RouterLink>
                    </div>
                    ...
                    <div class="hidden shrink-0 items-center gap-x-6 text-sm text-stone-600 sm:flex">
                        <div class="text-end">
                            <div class="font-bold text-stone-800"><Money :formatted="item.total_after_vat_formatted" /></div>
                            <div class="mt-0.5 text-xs text-stone-400 inline-flex items-baseline gap-1">مدفوع <Money :formatted="item.amount_paid_formatted" class="inline-flex" /></div>
                        </div>
