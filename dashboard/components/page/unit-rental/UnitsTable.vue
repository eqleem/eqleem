...
async function removeOne(item) {
    if (!confirm('هل أنت متأكد من حذف نوع الوحدة هذا؟')) {
        return;
    }

    await store.deleteUnits([item.id]);
...
async function cloneItem(id, uuid) {
    ...
    try {
        const clone = await store.cloneUnit(uuid);
        if (clone?.uuid) {
            router.push(`/manage/unit-rental/detail/${clone.uuid}`);
        }
    } catch (error) {
        notifyApiError(error, 'تعذر تكرار نوع الوحدة.');
    } finally {
...
async function toggleActive(item) {
    try {
        await store.toggleUnitActive(item.uuid, !item.active);
    } catch (error) {
        notifyApiError(error, 'تعذر تحديث حالة نوع الوحدة.');
    }
}
</script>
    ...
    <div class="divide-y divide-dotted divide-stone-200">
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <p class="text-stone-700">لا توجد أنواع وحدات.</p>
                <small class="text-stone-500">سيتم عرض أنواع الوحدات هنا بعد إضافتها.</small>
            </div>
            ...
            <div v-else>
                ...
                >
                    ...
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/manage/unit-rental/detail/${item.uuid}`" class="flex items-center gap-x-3 text-start">
                    ...
                    <div class="flex-none pe-6">
                        <Dropdown width="w-48">
                            ...
                            <a
                                v-if="unitPreviewUrl(item)"
                                :href="unitPreviewUrl(item)"
                                target="_blank"
