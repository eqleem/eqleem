    ...
    <div class="divide-y divide-dotted divide-gray-200">
        <div class="flex w-full items-center gap-x-4 bg-white p-3">
            <div class="flex-grow">
                <div class="relative text-sm text-gray-800">
                    ...
                    <input v-model="search" type="text" placeholder="ابحث .." class="block w-full bg-gray-100 rounded-lg border border-transparent py-1.5 ps-10 text-gray-800 ring-inset ring-gray-200 placeholder:text-gray-400 focus:border-primary-500 focus:outline-none sm:text-sm sm:leading-6">
                </div>
            </div>
        ...
        <div class="p-1">
            <div v-if="results.length === 0" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <p class="text-gray-700">لا توجد عناصر.</p>
                <small class="text-gray-500">سيتم عرض العناصر هنا بعد إضافتها.</small>
            </div>
            ...
            <div v-else>
                <div
                    ...
                    :key="item.id"
                    class="flex w-full items-center justify-between gap-x-4 px-4 py-3 last:rounded-b-2xl hover:bg-gray-50 sm:px-6"
                >
                    <RouterLink :to="`/manage/${contentType.slug}/detail/${item.id}`" class="flex min-w-0 flex-1 items-center gap-3">
                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gray-100 p-1.5">
