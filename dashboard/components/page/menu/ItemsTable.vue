...
function itemPreviewUrl(item) {
    ...
    return `${base}/menu/item/${item.slug}`;
}
...
async function removeSelected() {
    ...
    await store.deleteItems(selectedIds.value);
    selectedIds.value = [];
}
...
async function cloneItem(id, uuid) {
    ...
    try {
        const clone = await store.cloneItem(uuid);
        if (clone?.uuid) {
            router.push(`/manage/menu/detail/${clone.uuid}`);
        }
    } catch (error) {
        notifyApiError(error, 'تعذر تكرار الطبق.');
    } finally {
...
async function toggleActive(item) {
    try {
        await store.toggleItemActive(item.uuid, !item.active);
    } catch (error) {
        notifyApiError(error, 'تعذر تحديث حالة الطبق.');
    }
}
</script>
...
<template>
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-4 bg-white p-3">
            ...
            <Button label="طبق جديد" @click="openModal('add-menu-item')">
                <template #icon>
            ...
            <Modal title="إضافة طبق جديد" size="2xl" name="add-menu-item">
                <AddItem />
            </Modal>
        </div>
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <p class="text-stone-700">لا توجد أطباق.</p>
                <small class="text-stone-500">سيتم عرض الأطباق هنا بعد إضافتها.</small>
            </div>
            ...
            <div v-else>
                ...
                >
                    ...
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/manage/menu/detail/${item.uuid}`" class="flex items-center gap-x-3 text-start">
                    ...
                    <div class="flex-none pe-6">
                        <Dropdown width="w-48">
                            ...
                            <RouterLink
                                :to="`/manage/menu/detail/${item.uuid}`"
