...
function openAdd(parentId = null) {
    ...
    openModal('add-digital-product-category');
}

function openEdit(categoryId) {
    editingCategoryId.value = categoryId;
    openModal('edit-digital-product-category');
}
    ...
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-4 bg-white p-3">
            ...
            <Modal
                ...
                size="lg"
                name="add-digital-product-category"
            >
                <CategoryForm
                    :key="`add-${addingParentId ?? 'root'}`"
                    :default-parent-id="addingParentId"
                    modal-name="add-digital-product-category"
                />
            </Modal>
        </div>
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.categoriesEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <small class="text-stone-500">سيتم عرض تصنيفات الأعمال هنا بعد إضافتها.</small>
            </div>
        ...
        <Modal title="تعديل التصنيف" size="lg" name="edit-digital-product-category">
            <CategoryForm
                v-if="editingCategoryId"
                :key="`edit-${editingCategoryId}`"
                :category-id="editingCategoryId"
                modal-name="edit-digital-product-category"
            />
        </Modal>
    </div>
</template>
