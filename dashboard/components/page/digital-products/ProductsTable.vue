...
function productPreviewUrl(item) {
    ...
    return `${base}/digital-products/${item.slug}`;
}
...
async function toggleActive(item) {
    try {
        await store.toggleDigitalProductActive(item.uuid, !item.active);
    } catch (error) {
        notifyApiError(error, 'تعذر تحديث حالة المنتج.');
    }
}
</script>
...
<template>
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-4 bg-white p-3">
            ...
            <Button label="منتج جديد" @click="openModal('add-digital-product')">
                <template #icon>
            ...
            <Modal title="إضافة منتج رقمي جديد" size="2xl" name="add-digital-product">
                <AddProduct />
            </Modal>
        </div>
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <p class="text-stone-700">لا توجد منتجات رقمية.</p>
                <small class="text-stone-500">سيتم عرض المنتجات الرقمية هنا بعد إضافتها.</small>
            </div>
            ...
            <div v-else>
                ...
                >
                    ...
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/manage/digital-products/detail/${item.uuid}`" class="flex items-center gap-x-3 text-start">
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="mt-1 flex items-center gap-x-2">
                                    ...
                                    <p class="mt-1 flex items-center gap-1 text-xs text-stone-500">
                                        ...
                                        <span v-if="item.downloads_count > 0" class="text-xs text-stone-500">
                                            {{ item.downloads_count }} {{ item.downloads_count === 1 ? 'ملف' : 'ملفات' }}
                                        </span>
                    ...
                    <div class="flex-none pe-6">
                        <Dropdown width="w-48">
                            ...
                            <RouterLink
                                :to="`/manage/digital-products/detail/${item.uuid}`"
                            ...
                            <a
                                v-if="productPreviewUrl(item)"
                                :href="productPreviewUrl(item)"
                                target="_blank"
