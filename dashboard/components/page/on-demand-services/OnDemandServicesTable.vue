...
import AddOnDemandService from './AddOnDemandService.vue';
...
function servicePreviewUrl(item) {
    ...
    return `${base}/on-demand-services/${item.slug}`;
}

onMounted(() => {
    store.fetchOnDemandServices({ page: 1 });
});
...
async function removeSelected() {
    ...
    await store.deleteOnDemandServices(selectedIds.value);
    clearSelection();
}
...
async function removeOne(item) {
    ...
    await store.deleteOnDemandServices([item.id]);
    removeFromSelection(item.id);
}

async function toggleActive(item) {
    try {
        await store.toggleOnDemandServiceActive(item.uuid, !item.active);
    } catch (error) {
...
<template>
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-4 bg-white p-3">
            ...
            <Button label="خدمة جديد" @click="openModal('add-on-demand-service')">
                <template #icon>
            ...
            <Modal title="إضافة خدمة حسب الطلب" size="2xl" name="add-on-demand-service">
                <AddOnDemandService />
            </Modal>
        </div>
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <p class="text-stone-700">لا توجد خدمات.</p>
                <small class="text-stone-500">سيتم عرض الخدمات هنا بعد إضافتها.</small>
            </div>
            ...
            <div v-else>
                ...
                >
                    ...
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/manage/on-demand-services/detail/${item.uuid}`" class="flex items-center gap-x-3 text-start">
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="mt-1 flex items-center gap-x-2">
                                    ...
                                    <p class="mt-1 flex items-center gap-1 text-xs text-stone-500">
                                        ...
                                        <span v-if="item.price_label" class="text-xs font-medium text-stone-700">{{ item.price_label }}</span>
                    ...
                    <div class="flex-none pe-6">
                        <Dropdown width="w-48">
                            ...
                            <RouterLink
                                :to="`/manage/on-demand-services/detail/${item.uuid}`"
                ...
                <div v-if="store.hasPages" class="flex items-center justify-center gap-2 border-t border-dotted border-stone-200 p-3">
                    <Button
                        type="button"
                        variant="secondary"
                        label="السابق"
