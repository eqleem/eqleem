...
const parentOptions = computed(() => {
    const excluded = isEdit.value ? props.store.descendantIds(props.categoryId) : [];

    return props.store.parentOptionsFor(excluded);
});
...
function load() {
    ...
    if (isEdit.value) {
        const category = props.store.categories.find((item) => Number(item.id) === Number(props.categoryId));
...
watch(() => [props.categoryId, props.defaultParentId, props.store.categories], load, { immediate: true });
...
async function submit() {
    ...
    try {
        if (isEdit.value) {
            ...
            errors.slug = null;
            await props.store.updateCategory(props.categoryId, {
                name,
                slug,
                parent_id: form.parentId ? Number(form.parentId) : null,
            });
        } else {
            await props.store.createCategory({
                name,
                parent_id: form.parentId ? Number(form.parentId) : null,
            });
        }
