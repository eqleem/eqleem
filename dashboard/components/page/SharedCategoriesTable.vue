...
const props = defineProps({
    ...
    addModalName: { type: String, required: true },
    editModalName: { type: String, required: true },
    slugInfo: { type: String, default: 'يُستخدم في فلترة المنتجات عبر الرابط.' },
    emptyHint: { type: String, default: 'سيتم عرض تصنيفات الأعمال هنا بعد إضافتها.' },
});
...
watch(search, (value) => {
    ...
    searchTimer = setTimeout(() => {
        props.store.fetchCategories({ search: value });
    }, 300);
});
...
function openAdd(parentId = null) {
    ...
    openModal(props.addModalName);
}

function openEdit(categoryId) {
    editingCategoryId.value = categoryId;
    openModal(props.editModalName);
}
...
async function remove(categoryId) {
    ...
    await props.store.deleteCategory(categoryId);
}
...
async function onDrop(targetId) {
    ...
    const order = props.store.categories.map((item) => item.id);
    ...
    const byId = Object.fromEntries(props.store.categories.map((item) => [item.id, item]));
    props.store.categories = order.map((id) => byId[id]).filter(Boolean);

    await props.store.reorderCategories(order);
}
</script>
    ...
    <div class="divide-y divide-dotted divide-stone-200">
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.categoriesEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                <svg class="h-12 w-12 text-stone-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" d="M4 6h16M4 12h16M4 18h10" /></svg>
                <p class="text-stone-700">لا توجد تصنيفات.</p>
                <small class="text-stone-500">{{ emptyHint }}</small>
            </div>
            ...
            <ul v-else class="pb-4">
                <li
                    ...
                    :key="category.id"
                    class="group flex w-full items-center justify-between gap-x-4 hover:bg-stone-50 last:rounded-b-2xl"
                ...
                >
                    ...
                    >
                        <div class="flex min-w-0 items-center gap-x-3 pe-4">
                            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-stone-100">
                                <svg class="h-5 w-5 text-stone-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" d="M4 6h16M4 12h16M4 18h10" /></svg>
                            </div>
                            ...
                            >
                                <h2 class="truncate text-base text-stone-700">{{ category.name }}</h2>
                                <p v-if="category.description" class="mt-0.5 truncate text-xs text-stone-500">
                    ...
                    <div class="flex shrink-0 items-center gap-x-1 pe-6">
                        <button
                            type="button"
                            class="rounded-lg p-1.5 text-stone-400 opacity-0 transition pointer-events-none hover:bg-primary-50 hover:text-primary-600 group-hover:pointer-events-auto group-hover:opacity-100"
        ...
        <Modal title="تعديل التصنيف" size="lg" :name="editModalName">
            <CategoryForm
