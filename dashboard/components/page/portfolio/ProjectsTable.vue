...
function projectPreviewUrl(item) {
    ...
    return `${base}/portfolio/${item.slug}`;
}
...
async function removeOne(item) {
    ...
    await store.deleteProjects([item.id]);
...
async function cloneItem(id, uuid) {
    ...
    try {
        const clone = await store.cloneProject(uuid);
        if (clone?.uuid) {
            router.push(`/manage/portfolio/detail/${clone.uuid}`);
        }
    } catch (error) {
        notifyApiError(error, 'تعذر تكرار المشروع.');
    } finally {
...
async function toggleActive(item) {
    try {
        await store.toggleProjectActive(item.uuid, !item.active);
    } catch (error) {
        notifyApiError(error, 'تعذر تحديث حالة المشروع.');
    }
}
</script>
...
<template>
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-4 bg-white p-3">
            ...
            <Button label="مشروع جديد" @click="openModal('add-portfolio-project')">
                <template #icon>
            ...
            <Modal title="إضافة مشروع جديد" size="2xl" name="add-portfolio-project">
                <AddProject />
            </Modal>
        </div>
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <p class="text-stone-700">لا توجد مشاريع.</p>
                <small class="text-stone-500">سيتم عرض المشاريع هنا بعد إضافتها.</small>
            </div>
            ...
            <div v-else>
                ...
                >
                    ...
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/manage/portfolio/detail/${item.uuid}`" class="flex items-center gap-x-3 text-start">
                    ...
                    <div class="flex-none pe-6">
                        <Dropdown width="w-48">
                            ...
                            <RouterLink
                                :to="`/manage/portfolio/detail/${item.uuid}`"
                            ...
                            <a
                                v-if="projectPreviewUrl(item)"
                                :href="projectPreviewUrl(item)"
                                target="_blank"
