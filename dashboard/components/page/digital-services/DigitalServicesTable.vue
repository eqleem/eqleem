...
function servicePreviewUrl(item) {
    ...
    return `${base}/digital-services/${item.slug}`;
}
...
async function cloneItem(id, uuid) {
    ...
    try {
        const clone = await store.cloneDigitalService(uuid);
        if (clone?.uuid) {
            router.push(`/manage/digital-services/detail/${clone.uuid}`);
        }
    } catch (error) {
        notifyApiError(error, 'تعذر تكرار الخدمة.');
    } finally {
...
async function toggleActive(item) {
    try {
        await store.toggleDigitalServiceActive(item.uuid, !item.active);
    } catch (error) {
...
<template>
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-4 bg-white p-3">
            ...
            <Button label="خدمة جديد" @click="openModal('add-digital-service')">
                <template #icon>
            ...
            <Modal title="إضافة خدمة جديد" size="2xl" name="add-digital-service">
                <AddDigitalService />
            </Modal>
        </div>
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <p class="text-stone-700">لا توجد خدمةات.</p>
                <small class="text-stone-500">سيتم عرض الخدمةات هنا بعد إضافتها.</small>
            </div>
            ...
            <div v-else>
                ...
                >
                    ...
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/manage/digital-services/detail/${item.uuid}`" class="flex items-center gap-x-3 text-start">
                    ...
                    <div class="flex-none pe-6">
                        <Dropdown width="w-48">
                            ...
                            <RouterLink
                                :to="`/manage/digital-services/detail/${item.uuid}`"
