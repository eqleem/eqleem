...
function productPreviewUrl(item) {
    ...
    return `${base}/store/product/${item.slug}`;
}
...
async function removeOne(item) {
    ...
    await store.deleteProducts([item.id]);
    removeFromSelection(item.id);
}

async function toggleActive(item) {
    try {
        await store.toggleProductActive(item.uuid, !item.active);
    } catch (error) {
...
<template>
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-4 bg-white p-3">
            ...
            <Button label="منتج جديد" @click="openModal('add-store-product')">
                <template #icon>
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <p class="text-stone-700">لا توجد منتجات.</p>
                <small class="text-stone-500">سيتم عرض المنتجات هنا بعد إضافتها.</small>
            </div>
            ...
            <div v-else>
                ...
                >
                    ...
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/manage/store/detail/${item.uuid}`" class="flex items-center gap-x-3 text-start">
                    ...
                    <div class="flex-none pe-6">
                        <Dropdown width="w-48">
                            ...
                            <RouterLink
                                :to="`/manage/store/detail/${item.uuid}`"
