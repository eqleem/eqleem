...
async function removeSelected() {
    ...
    await store.deletePosts(selectedIds.value);
    clearSelection();
}
...
async function removeOne(item) {
    ...
    await store.deletePosts([item.id]);
    removeFromSelection(item.id);
}

async function toggleActive(item) {
    try {
        await store.togglePostActive(item.uuid, !item.active);
    } catch (error) {
        notifyApiError(error, 'تعذر تحديث حالة التدوينة.');
    }
}
</script>
...
<template>
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-4 bg-white p-3">
            ...
            <Button label="تدوينة جديدة" @click="openModal('add-blog-post')">
                <template #icon>
            ...
            <Modal title="إضافة تدوينة جديدة" size="2xl" name="add-blog-post">
                <AddPost />
            </Modal>
        </div>
        ...
        <div class="relative p-1">
            ...
            <div v-else-if="store.isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <p class="text-stone-700">لا توجد تدوينات.</p>
                <small class="text-stone-500">سيتم عرض التدوينات هنا بعد إضافتها.</small>
            </div>
            ...
            <div v-else>
                ...
                >
                    ...
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/manage/blog/detail/${item.uuid}`" class="flex items-center gap-x-3 text-start">
                    ...
                    <div class="flex-none pe-6">
                        <Dropdown width="w-48">
                            ...
                            <RouterLink
                                :to="`/manage/blog/detail/${item.uuid}`"
                            ...
                            <a
                                v-if="postPreviewUrl(item)"
                                :href="postPreviewUrl(item)"
                                target="_blank"
