...
const { items, meta, search, loading, loaded, error, isEmpty, hasPages } = storeToRefs(paymentsStore);
...
watch(searchInput, (value) => {
    ...
    searchTimer = setTimeout(() => paymentsStore.setSearch(value), 300);
});
    ...
    <div class="divide-y divide-dotted divide-stone-200">
        <div class="flex w-full items-center gap-x-7 bg-white p-3">
            <div class="flex-grow bg-stone-100 rounded-lg">
                <div class="relative col-span-3 text-sm text-stone-800">
                    ...
                    <input v-model="searchInput" type="text" placeholder="ابحث .." class="block w-full rounded-lg border border-transparent py-1.5 ps-10 text-stone-800 ring-inset ring-stone-200 placeholder:text-stone-400 focus:border-primary-500 focus:outline-none sm:text-sm sm:leading-6">
                </div>
            </div>
        </div>
        ...
        <div class="relative p-1">
            <div
                ...
                class="animate-pulse"
                aria-busy="true"
                aria-label="جاري تحميل عمليات الدفع"
            >
                <div
                    v-for="n in 6"
                    :key="`skeleton-${n}`"
                ...
                >
                    <div class="min-w-0 flex-1 space-y-2 py-3">
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                            <div
                                class="h-5 rounded-md bg-stone-200"
                        ...
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                            ...
                            ></div>
                            <div class="h-5 w-16 rounded-md bg-stone-100"></div>
                            <div class="h-3 w-24 rounded-md bg-stone-100"></div>
                        </div>
                    </div>

                    <div class="hidden shrink-0 items-center gap-x-6 sm:flex">
                        <div class="space-y-1.5 text-end">
                            <div class="ms-auto h-4 w-20 rounded-md bg-stone-200"></div>
                            <div class="ms-auto h-3 w-14 rounded-md bg-stone-100"></div>
                        </div>
                        <div class="min-w-24 space-y-1.5 text-end">
                            <div class="ms-auto h-4 w-16 rounded-md bg-stone-200"></div>
                            <div class="ms-auto h-3 w-12 rounded-md bg-stone-100"></div>
                        </div>
                    </div>
            ...
            <div v-else-if="error" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                ...
                <button type="button" class="rounded-lg border bg-white px-3 py-1.5 text-sm" @click="paymentsStore.fetchList({ page: meta.current_page })">إعادة المحاولة</button>
            </div>

            <div v-else-if="isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                <svg class="h-12 w-12 p-0.5 text-stone-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M5 21V5a1 1 0 0 1 1.5-.9L8 5l1.5-1 1.5 1L12 5l1.5-1 1.5 1L16 5l1.5-.9A1 1 0 0 1 19 5v16l-2-1-2 1-2-1-2 1-2-1-2 1Z" /></svg>
                <p class="text-stone-700">لا توجد عمليات دفع.</p>
                <small class="text-stone-500">ستظهر عمليات الدفع هنا بعد إتمام أي عملية شراء أو اشتراك.</small>
            </div>

            <div v-else-if="items.length > 0" class="divide-y-2 divide-dotted divide-stone-200/50">
                <div v-for="item in items" :key="item.id" class="flex w-full items-center justify-between gap-x-4 px-4 last:rounded-b-2xl hover:bg-stone-50 sm:px-6">
                    <div class="min-w-0 flex-1 py-3">
                        <RouterLink :to="`/payments/${item.uuid}`" class="block">
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                                <h2 class="text-lg font-semibold text-stone-700">#{{ item.id }}</h2>
                                <Badge :color="item.status_color">{{ item.status_label }}</Badge>
                                <Badge color="gray">{{ item.reason_label }}</Badge>
                            </div>
                            <div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-stone-500">
                                <span v-if="item.payer" class="truncate">{{ item.payer }}</span>
                                <span v-if="item.email" class="inline-flex items-center gap-x-1 truncate rounded-md bg-stone-100 p-1 px-2 text-xs">{{ item.email }}</span>
                                <span v-if="item.gateway_label" class="inline-flex items-center gap-x-1 rounded-md bg-stone-100 p-1 px-2 text-xs">{{ item.gateway_label }}</span>
                    ...
                    <div class="hidden shrink-0 items-center gap-x-6 text-sm text-stone-600 sm:flex">
                        <div class="text-end">
                            <div class="font-bold text-stone-800"><Money :formatted="item.amount_formatted" /></div>
                            <div class="mt-0.5 text-xs text-stone-400">{{ item.source_type_label }}</div>
                        </div>
                        <div class="min-w-24 text-end">
                            <div>{{ item.date }}</div>
                            <div class="mt-0.5 text-xs text-stone-400" dir="ltr">{{ item.time }}</div>
                        </div>
                    </div>
