    ...
    <div class="divide-y divide-dotted divide-stone-200">
        ...
        <div class="relative p-1">
                ...
                >
                    <div class="min-w-0 flex-1 space-y-2 py-3.5">
                        ...
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                            ...
                            ></div>
                            <div class="h-4 w-20 rounded-md bg-stone-100"></div>
                        </div>
                    </div>
            ...
            <div v-else-if="isEmpty" class="flex flex-col items-center justify-center gap-2 p-10 text-center">
                <Icon name="clipboard" class="h-12 w-12 p-0.5 text-stone-400" />
                <p class="text-stone-700">لا توجد ردود.</p>
                <small class="text-stone-500">سيتم عرض ردود النماذج هنا بعد إرسالها من الموقع.</small>
            </div>
            ...
            <div v-else-if="items.length > 0" class="divide-y-2 divide-dotted divide-stone-200/50">
                <div
                    ...
                    :class="item.unread ? 'border-primary-500 bg-primary-50/60 hover:bg-primary-50' : 'border-transparent hover:bg-stone-50'"
                >
                    <div class="min-w-0 flex-1 py-3.5">
                        <RouterLink :to="`/form-submissions/${item.id}`" class="block">
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                                ...
                                <h2 class="text-lg font-semibold" :class="item.unread ? 'text-stone-900' : 'text-stone-700'">#{{ item.id }}</h2>
                                <Badge v-if="item.unread" color="blue">غير مقروء</Badge>
                                <Badge v-else :color="item.status_color">{{ item.status_label }}</Badge>
                                <span v-if="item.form_title" class="inline-flex items-center gap-x-1 truncate rounded-md bg-white/80 p-1 px-2 text-xs text-stone-600 ring-1 ring-stone-200/80">
                                    <Icon name="clipboard" class="h-3.5 w-3.5 text-stone-400" />
                                    {{ item.form_title }}
                                </span>
                            </div>
                            <div class="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm" :class="item.unread ? 'text-stone-700' : 'text-stone-500'">
                                <template v-if="item.client">
                                    <span class="truncate font-medium">{{ item.client.name }}</span>
                                    <span v-if="item.client.email" class="inline-flex items-center gap-x-1 truncate rounded-md bg-white/80 p-1 px-2 text-xs ring-1 ring-stone-200/80">{{ item.client.email }}</span>
                                ...
                                <span v-else class="text-stone-400">زائر</span>
                            </div>
                        </RouterLink>
                    </div>

                    <div class="hidden shrink-0 items-center gap-x-6 text-sm sm:flex" :class="item.unread ? 'text-stone-700' : 'text-stone-600'">
