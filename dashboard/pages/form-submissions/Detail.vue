                ...
                <Section title="ملخص الرد" icon="clipboard">
                    <div class="space-y-3 p-5">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-medium text-gray-800">{{ submission.fields_count }}</span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-500">عدد الردود</span>
                            <span class="font-medium text-gray-800">{{ submission.replies_count }}</span>
                        </div>
                        ...
                        <div class="space-y-2 border-t border-gray-100 pt-3">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-medium text-gray-800">{{ submission.submitted }}</span>
                            </div>
                    ...
                    <div class="p-5">
                        <template v-if="submission.client">
                            <div class="flex items-center gap-3">
                                <img :src="submission.client.avatar" :alt="submission.client.name" class="h-12 w-12 shrink-0 rounded-full bg-gray-100 object-cover">
                                <div class="min-w-0">
                                    <p class="truncate font-semibold text-gray-900">{{ submission.client.name }}</p>
                                    <p v-if="submission.client.email" class="truncate text-sm text-gray-500">{{ submission.client.email }}</p>
                                    <p v-if="submission.client.phone" class="text-sm text-gray-500" dir="ltr">{{ submission.client.phone }}</p>
                                </div>
                            </div>
                        ...
                        <div v-else class="flex flex-col items-center py-4 text-center">
                            ...
                            <p class="mt-3 text-sm font-semibold text-gray-700">زائر</p>
            ...
            <div class="space-y-6 lg:order-2 lg:col-span-2">
                <Section title="الحقول" icon="list">
                    <div class="divide-y divide-gray-50 p-5">
                        <div v-if="submission.fields.length === 0" class="py-6 text-center text-sm text-gray-500">لا توجد حقول.</div>
                        <div v-for="field in submission.fields" :key="field.id || field.name" class="py-4 first:pt-0 last:pb-0">
                            <p class="text-xs text-gray-400">{{ field.label || field.name }}</p>
                            <p class="mt-1 text-sm font-medium text-gray-800 whitespace-pre-wrap">
                ...
                <Section title="الردود" icon="message">
                    <div class="p-5">
                        <div v-if="submission.replies.length === 0" class="py-6 text-center text-sm text-gray-500">لا توجد ردود بعد.</div>
                        <div v-else class="space-y-4">
                            <div v-for="reply in submission.replies" :key="reply.id" class="rounded-lg bg-gray-50 p-4">
                                <div class="flex items-center justify-between gap-3">
                                    <p class="text-sm font-semibold text-gray-800">{{ reply.user || 'فريق المتجر' }}</p>
                                    <p class="text-xs text-gray-400">{{ reply.created }}</p>
                                </div>
                                <p class="mt-2 text-sm leading-relaxed text-gray-700 whitespace-pre-wrap">{{ reply.body }}</p>
                            </div>
                        </div>
                    </div>
                </Section>
