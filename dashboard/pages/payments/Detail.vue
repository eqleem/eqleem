            ...
            <div class="space-y-6 lg:order-1">
                ...
                <Section title="معلومات تقنية" icon="info">
                    <div class="space-y-3 p-5 text-sm">
                        ...
                        <div><p class="text-xs text-gray-400">تاريخ الإنشاء</p><p class="mt-0.5 text-xs font-medium text-gray-800">{{ payment.date }} {{ payment.time }}</p></div>
                    </div>
                </Section>
            </div>
                ...
                <Section title="تفاصيل الدفع" icon="receipt">
                    <div class="p-5">
                        <dl class="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
                            ...
                            <div><dt class="mb-1 text-xs text-gray-400">الحالة</dt><dd><Badge :color="payment.status_color">{{ payment.status_label }}</Badge></dd></div>
                            <div><dt class="mb-1 text-xs text-gray-400">نوع العملية</dt><dd class="text-sm text-gray-800">{{ payment.reason_label }}</dd></div>
                            <div><dt class="mb-1 text-xs text-gray-400">تاريخ العملية</dt><dd class="text-sm text-gray-800">{{ payment.date }} <span class="text-gray-400" dir="ltr">{{ payment.time }}</span></dd></div>
                            <div><dt class="mb-1 text-xs text-gray-400">بوابة الدفع</dt><dd class="flex items-center gap-1.5 text-sm text-gray-800"><Icon name="bank" class="h-4 w-4 text-gray-400" /> {{ payment.gateway_label }}</dd></div>
                            <div><dt class="mb-1 text-xs text-gray-400">طريقة الدفع</dt><dd class="flex items-center gap-1.5 text-sm text-gray-800"><Icon name="card" class="h-4 w-4 text-gray-400" /> {{ payment.source_type_label }}</dd></div>
