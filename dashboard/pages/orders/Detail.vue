...
<template>
    <Container :title="`الطلبات / #${order?.number ?? '...'}`" back-route="/orders">
        ...
        <div v-else-if="error && !order" class="flex flex-col items-center justify-center gap-3 rounded-xl bg-white p-16 text-center">
            ...
            <button type="button" class="rounded-lg border bg-white px-3 py-1.5 text-sm text-stone-700 hover:bg-stone-100" @click="loadOrder(route.params.uuid)">
                إعادة المحاولة
            </button>
        </div>
        ...
        <div v-else-if="order" class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <Section title="ملخص الطلب" icon="receipt">
                    <div class="space-y-3 p-5">
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-medium text-stone-800"><Money :formatted="order.subtotal_formatted" /></span>
                        </div>
                        <div v-if="order.discount_total > 0" class="flex items-center justify-between text-sm">
                            <span class="text-stone-500">الخصومات</span>
                        ...
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-stone-500">الضريبة</span>
                            <span class="font-medium text-stone-800"><Money :formatted="order.tax_total_formatted" /></span>
                        </div>
                        ...
                        <div class="space-y-2 border-t border-stone-100 pt-3">
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-stone-500">المدفوع</span>
                            ...
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-medium" :class="order.due_total > 0 ? 'text-amber-700' : 'text-stone-800'"><Money :formatted="order.due_total_formatted" /></span>
                            </div>
                        </div>
                    </div>
                </Section>
                    ...
                    <div class="p-5">
                        <template v-if="order.client">
                            <div class="flex items-center gap-3">
                                <img :src="order.client.avatar" :alt="order.client.name" class="h-12 w-12 shrink-0 rounded-full bg-stone-100 object-cover">
                                <div class="min-w-0">
                                    <p class="truncate font-semibold text-stone-900">{{ order.client.name }}</p>
                                    <p class="truncate text-sm text-stone-500">{{ order.client.email }}</p>
                                    <p class="text-sm text-stone-500" dir="ltr">{{ order.client.phone }}</p>
                                </div>
                            </div>
                        ...
                        <div v-else class="flex flex-col items-center py-4 text-center">
                            ...
                            <p class="mt-1 text-xs text-stone-400">طلب بدون حساب عميل</p>
                        </div>
                    </div>
                </Section>
            </div>
            ...
            <div class="space-y-6 lg:order-2 lg:col-span-2">
                <Section title="تفاصيل الطلب" icon="package">
                    ...
                    <div class="p-5">
                        <dl class="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
                            <div>
                                <dt class="mb-1 text-xs text-stone-400">رقم الطلب</dt>
                                <dd class="text-sm font-semibold text-stone-900">#{{ order.number }}</dd>
                            </div>
                            <div>
                                <dt class="mb-1 text-xs text-stone-400">حالة الطلب</dt>
                                <dd><Badge :color="order.status_color">{{ order.status_label }}</Badge></dd>
                            </div>
                            <div>
                                <dt class="mb-1 text-xs text-stone-400">تاريخ الطلب</dt>
                                <dd class="text-sm text-stone-800">{{ order.created }}</dd>
                            </div>
                            <div>
                                <dt class="mb-1 text-xs text-stone-400">حالة الدفع</dt>
                                <dd><Badge :color="order.payment_status_color">{{ order.payment_status_label }}</Badge></dd>
                            </div>
                            <div>
                                <dt class="mb-1 text-xs text-stone-400">مصدر الطلب</dt>
                                <dd class="flex items-center gap-1.5 text-sm text-stone-800"><Icon name="store" class="h-4 w-4 text-stone-400" /> {{ order.channel_label }}</dd>
                            </div>
                            <div>
                                <dt class="mb-1 text-xs text-stone-400">طريقة الدفع</dt>
                                <dd class="flex items-center gap-1.5 text-sm text-stone-800"><Icon name="card" class="h-4 w-4 text-stone-400" /> {{ order.payment_method_label }}</dd>
                            </div>
                        </dl>
                    </div>
                </Section>
                ...
                <Section :title="`العناصر (${order.items.length})`" icon="cart">
                                ...
                                <tr v-for="item in order.items" :key="item.id">
                                    <td class="px-5 py-4">
                                        <div class="flex items-start gap-3">
                                            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-stone-100 text-stone-400"><Icon name="package" class="h-5 w-5" /></div>
                                            <div class="min-w-0 space-y-1">
                                                    ...
                                                    <div class="mt-1.5 space-y-1 text-xs text-stone-500">
                                                        ...
                                                        <template v-else-if="item.type === 'unit_rental'">
                                                            <p v-if="item.booking.dates_label">
                                                                <span class="text-stone-400">التواريخ:</span>
                                                                {{ item.booking.dates_label }}
                                                            </p>
                                                            <p v-if="item.booking.duration_label" class="text-stone-400">
                                                ...
                                                <div v-else class="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-stone-500">
                                    ...
                                    <td class="whitespace-nowrap px-3 py-4 text-stone-600"><Money :formatted="item.unit_price_formatted" /></td>
                                    <td class="px-3 py-4 text-center text-stone-800">{{ item.qty }}</td>
                                    <td class="whitespace-nowrap px-5 py-4 text-end font-semibold text-stone-900"><Money :formatted="item.line_total_formatted" /></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <div class="border-t border-stone-100 px-5 py-4">
                        <div class="ms-auto max-w-xs space-y-2">
                            <div class="flex items-center justify-between text-sm"><span class="text-stone-500">المجموع الفرعي</span><span class="text-stone-800"><Money :formatted="order.subtotal_formatted" /></span></div>
                            <div v-if="order.discount_total > 0" class="flex items-center justify-between text-sm"><span class="text-stone-500">الخصومات</span><span class="text-red-600 inline-flex items-baseline">−<Money :formatted="order.discount_total_formatted" class="inline-flex" /></span></div>
                            <div class="flex items-center justify-between text-sm"><span class="text-stone-500">الضريبة</span><span class="text-stone-800"><Money :formatted="order.tax_total_formatted" /></span></div>
                            <div class="flex items-center justify-between border-t border-stone-100 pt-2"><span class="font-semibold text-stone-800">الإجمالي</span><span class="text-lg font-bold text-primary-700"><Money :formatted="order.grand_total_formatted" /></span></div>
                        </div>
                    </div>
                </Section>
                ...
                <Section title="المدفوعات" icon="coin">
                    ...
                    <div class="p-5">
                        <div v-if="order.payments.length === 0" class="flex flex-col items-center gap-2 py-6 text-center">
                            <Icon name="coin" class="h-12 w-12 p-0.5 text-stone-400" />
                            <p class="text-stone-700">لا توجد مدفوعات بعد.</p>
                            <small class="text-stone-500">سجّل دفعة لإنشاء فاتورة وربطها بهذا الطلب.</small>
                        </div>
                            ...
                            <div v-for="payment in order.payments" :key="payment.id" class="flex flex-col gap-3 py-4 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between">
                                <div class="min-w-0">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <RouterLink :to="`/payments/${payment.uuid}`" class="text-sm font-semibold text-stone-800 transition hover:text-primary-600">{{ payment.method }}</RouterLink>
                ...
                <Section title="سجل النشاط" icon="history">
                    <div class="p-5">
                        <div v-if="order.activity.length === 0" class="py-6 text-center text-sm text-stone-500">لا يوجد نشاط بعد.</div>
                        <div v-else class="space-y-0">
                            <div v-for="(entry, index) in order.activity" :key="entry.key" class="relative flex gap-4 pb-6 last:pb-0">
                                <span v-if="index < order.activity.length - 1" class="absolute bottom-0 top-8 w-px bg-stone-200" style="inset-inline-start: 0.875rem;"></span>
                                <div class="relative z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-full" :class="entry.type === 'status' ? 'bg-primary-50 text-primary-600' : 'bg-stone-100 text-stone-500'">
                                    <Icon :name="entry.type === 'status' ? 'refresh' : 'history'" class="h-3.5 w-3.5" />
                                </div>
                                <div class="min-w-0 flex-1 pt-0.5">
                                    <p class="text-sm text-stone-800">{{ entry.title }}</p>
                                    <Badge v-if="entry.type === 'status'" :color="entry.status_color" class="mt-1">{{ entry.status_label }}</Badge>
                                    <p class="mt-1 text-xs text-stone-400">{{ entry.date }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
