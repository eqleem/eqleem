...
onMounted(async () => {
    try {
        const product = await store.fetchProduct(uuid.value);
        loadForm(product);
    } catch (error) {
...
watch(() => route.params.id, async (id) => {
    ...
    try {
        const product = await store.fetchProduct(String(id));
        loadForm(product);
    } catch (error) {
        notFound.value = error instanceof ApiError && error.status === 404;
    }
...
async function persist({ close = false } = {}) {
    ...
    try {
        const product = await store.updateProduct(uuid.value, payload);
            ...
            <div class="relative z-20 flex items-center justify-between gap-4 border-b border-stone-200 bg-stone-200/70 px-4 py-3">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="flex min-w-0 items-center gap-2 text-sm text-stone-700">
                        ...
                        <span class="truncate text-stone-600 hidden md:inline">تحرير المنتج</span>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-4">
                    ...
                    >
                        ...
                        <Input
                            ...
                            min="0"
                            placeholder="0.000"
                            info="ضروري فقط إذا كنت تشحن المنتجات للعميل من خلال شركات الشحن لحسبة تكلفة الشحن حسب الوزن."
                        />
