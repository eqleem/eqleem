...
const editorUploadUrl = computed(() => `/api/digital-services/${uuid.value}/editor-images`);
const categories = computed(() => store.detail?.category_options ?? []);
const slugPrefix = computed(() => store.detail?.slug_prefix ?? '/digital-services/');
...
function loadForm(service, { syncEditor = true } = {}) {
    ...
    form.deliveryDays = service.delivery_days ?? '';
...
onMounted(async () => {
    try {
        const service = await store.fetchDigitalService(uuid.value);
        loadForm(service);
    } catch (error) {
...
watch(() => route.params.id, async (id) => {
    ...
    try {
        const service = await store.fetchDigitalService(String(id));
        loadForm(service);
    } catch (error) {
        notFound.value = error instanceof ApiError && error.status === 404;
    }
...
async function persist({ close = false } = {}) {
    ...
    if (form.deliveryDays !== '') {
        payload.delivery_days = Number(form.deliveryDays);
    }

    try {
        const service = await store.updateDigitalService(uuid.value, payload);

        if (close) {
            router.push('/manage/digital-services');
            return;
        }
            ...
            <div class="relative z-20 flex items-center justify-between gap-4 border-b border-stone-200 bg-stone-200/70 px-4 py-3">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="flex min-w-0 items-center gap-2 text-sm text-stone-700">
                        ...
                        <span class="truncate text-stone-600 hidden md:inline">تحرير الخدمة الرقمية</span>
                    </div>
                </div>
            </div>
