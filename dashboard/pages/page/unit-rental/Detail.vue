...
function loadForm(unit, { syncEditor = true } = {}) {
    if (!unit) {
        return;
    }
    ...
    form.published = Boolean(unit.published);
...
onMounted(async () => {
    try {
        const unit = await store.fetchUnit(uuid.value);
        loadForm(unit);
    } catch (error) {
...
watch(() => route.params.id, async (id) => {
    ...
    try {
        const unit = await store.fetchUnit(String(id));
        loadForm(unit);
    } catch (error) {
        notFound.value = error instanceof ApiError && error.status === 404;
    }
            ...
            <div class="relative z-20 flex items-center justify-between gap-4 border-b border-stone-200 bg-stone-200/70 px-4 py-3">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="flex min-w-0 items-center gap-2 text-sm text-gray-700">
                        ...
                        <span class="truncate text-gray-600 hidden md:inline">تحرير نوع الوحدة</span>
                    </div>
                </div>
            ...
            <Form class="!rounded-none !p-4 md:!p-6" @submit="save">
                ...
                >
                    <div class="relative rounded-md bg-gray-100/75 p-1 lg:flex lg:items-start lg:gap-x-2">
                        <span class="inline-block w-36 flex-shrink-0 p-2 text-sm font-semibold text-gray-500">التصنيف</span>
                    ...
                    <div class="relative rounded-md bg-gray-100/75 p-1 lg:flex lg:items-start lg:gap-x-2">
                        <span class="inline-block w-36 flex-shrink-0 p-2 text-sm font-semibold text-gray-500">مخزون الوحدات</span>
                        <div class="w-full space-y-1.5 p-2">
                            <label
                            ...
                            <p v-if="calendars.length === 0" class="text-xs text-gray-400">لا توجد وحدات مخزون بعد.</p>
                            <p class="text-xs text-gray-400">اربط نوع الوحدة بوحدات المخزون المتاحة لحجز المواعيد حسب جداولها.</p>
                        </div>
                    </div>
