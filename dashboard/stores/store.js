...
export const useStoreStore = defineStore('storeCatalog', {
    state: () => ({
        type: contentTypeBySlug('store'),
        items: [],
        meta: emptyMeta(),
        search: '',
        loading: false,
    ...
    actions: {
        async fetchProducts({ page = 1, search } = {}) {
            ...
            try {
                ...
                const payload = await api(`/store?${params.toString()}`);
                ...
                this.error = error instanceof ApiError ? error.message : 'تعذر تحميل المنتجات.';
        ...
        async createProduct(title) {
            ...
            try {
                ...
                this.error = error instanceof ApiError ? error.message : 'تعذر إنشاء المنتج.';
                redirectIfUnauthorized(error);
                throw error;
            } finally {
                this.saving = false;
        ...
        async fetchProduct(uuid) {
            ...
            try {
                const payload = await api(`/store/${uuid}`);
                this.detail = payload?.data ?? null;

                return this.detail;
            } catch (error) {
                this.detail = null;
                this.detailError = error instanceof ApiError ? error.message : 'تعذر تحميل المنتج.';
                redirectIfUnauthorized(error);
                throw error;
            } finally {
        ...
        async deleteProducts(ids) {
            ...
            try {
                ...
                await this.fetchProducts({ page: this.meta.current_page || 1 });
            } catch (error) {
                this.error = error instanceof ApiError ? error.message : 'تعذر حذف المنتجات.';
                redirectIfUnauthorized(error);
                throw error;
            } finally {
                this.saving = false;
        ...
        async uploadImage(uuid, file) {
            ...
            const payload = await api(`/store/${uuid}/images`, {
                method: 'POST',
                body,
            });
        ...
        async reorderImages(uuid, order) {
            const payload = await api(`/store/${uuid}/images/reorder`, {
                method: 'PUT',
                body: { order },
            });
        ...
        async deleteImage(uuid, mediaId) {
            const payload = await api(`/store/${uuid}/images/${mediaId}`, {
                method: 'DELETE',
            });
        ...
        async fetchCategories({ search, force = false } = {}) {
            ...
            try {
                ...
                const payload = await api(`/store/categories${qs ? `?${qs}` : ''}`);
        ...
        async createCategory(data) {
            ...
            try {
                const payload = await api('/store/categories', {
                    method: 'POST',
                    body: data,
                });
        ...
        async updateCategory(id, data) {
            ...
            try {
                const payload = await api(`/store/categories/${id}`, {
                    method: 'PUT',
                    body: data,
                });
        ...
        async reorderCategories(order) {
            ...
            try {
                const payload = await api('/store/categories/reorder', {
                    method: 'PUT',
                    body: { order },
                });
        ...
        async fetchSettings({ force = false } = {}) {
            ...
            try {
                const payload = await api('/store/settings');
                this.settings = {
