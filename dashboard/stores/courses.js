...
export const useCoursesStore = defineStore('coursesCatalog', {
    state: () => ({
        type: contentTypeBySlug('courses'),
        items: [],
        meta: emptyMeta(),
        search: '',
        loading: false,
    ...
    actions: {
        async fetchCourses({ page = 1, search } = {}) {
            if (search !== undefined) {
                this.search = search;
            }
            ...
            try {
                ...
                const payload = await api(`/courses?${params.toString()}`);
                ...
                this.error = error instanceof ApiError ? error.message : 'تعذر تحميل الدورات.';
        ...
        async createCourse(title) {
            ...
            try {
                ...
                this.error = error instanceof ApiError ? error.message : 'تعذر إنشاء الدورة.';
                redirectIfUnauthorized(error);
                throw error;
            } finally {
                this.saving = false;
        ...
        async fetchCourse(uuid) {
            this.detailLoading = true;
            this.detailError = null;

            try {
                const payload = await api(`/courses/${uuid}`);
                this.detail = payload?.data ?? null;

                return this.detail;
            } catch (error) {
                this.detail = null;
                this.detailError = error instanceof ApiError ? error.message : 'تعذر تحميل الدورة.';
                redirectIfUnauthorized(error);
                throw error;
            } finally {
        ...
        async deleteCourses(ids) {
            ...
            try {
                ...
                await this.fetchCourses({ page: this.meta.current_page || 1 });
            } catch (error) {
                this.error = error instanceof ApiError ? error.message : 'تعذر حذف الدورات.';
                redirectIfUnauthorized(error);
                throw error;
            } finally {
                this.saving = false;
        ...
        async uploadCoverImage(uuid, file) {
            const body = new FormData();
            body.append('file', file);

            const payload = await api(`/courses/${uuid}/cover-image`, {
                method: 'POST',
                body,
            });
        ...
        async deleteCoverImage(uuid, mediaId) {
            const payload = await api(`/courses/${uuid}/cover-image/${mediaId}`, {
                method: 'DELETE',
            });
        ...
        async uploadLessonFile(uuid, { chapterId, lessonId, file }) {
            const body = new FormData();
            body.append('file', file);
            body.append('chapter_id', chapterId);
            body.append('lesson_id', lessonId);

            const payload = await api(`/courses/${uuid}/lesson-files`, {
                method: 'POST',
                body,
            });
        ...
        async deleteLessonFile(uuid, mediaId) {
            await api(`/courses/${uuid}/lesson-files/${mediaId}`, {
                method: 'DELETE',
            });
        },
        ...
        async fetchCategories({ search } = {}) {
            ...
            try {
                ...
                const payload = await api(`/courses/categories${qs ? `?${qs}` : ''}`);
        ...
        async createCategory(data) {
            ...
            try {
                const payload = await api('/courses/categories', {
                    method: 'POST',
                    body: data,
                });
        ...
        async updateCategory(id, data) {
            ...
            try {
                const payload = await api(`/courses/categories/${id}`, {
                    method: 'PUT',
                    body: data,
                });
        ...
        async reorderCategories(order) {
            ...
            try {
                const payload = await api('/courses/categories/reorder', {
                    method: 'PUT',
                    body: { order },
                });
        ...
        async fetchSettings({ force = false } = {}) {
            ...
            try {
                const payload = await api('/courses/settings');
                this.settings = {
