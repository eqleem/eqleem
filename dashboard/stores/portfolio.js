...
export const usePortfolioStore = defineStore('portfolio', {
    state: () => ({
        type: contentTypeBySlug('portfolio'),
        items: [],
        meta: emptyMeta(),
        search: '',
        loading: false,
    ...
    actions: {
        async fetchProjects({ page = 1, search } = {}) {
            ...
            try {
                ...
                const payload = await api(`/portfolio?${params.toString()}`);
                ...
                this.error = error instanceof ApiError ? error.message : 'تعذر تحميل المشاريع.';
        ...
        async createProject(title) {
            ...
            try {
                ...
                this.error = error instanceof ApiError ? error.message : 'تعذر إنشاء المشروع.';
                redirectIfUnauthorized(error);
                throw error;
            } finally {
                this.saving = false;
        ...
        async fetchProject(uuid) {
            ...
            try {
                const payload = await api(`/portfolio/${uuid}`);
                this.detail = payload?.data ?? null;

                return this.detail;
            } catch (error) {
                this.detail = null;
                this.detailError = error instanceof ApiError ? error.message : 'تعذر تحميل المشروع.';
                redirectIfUnauthorized(error);
                throw error;
            } finally {
        ...
        async deleteProjects(ids) {
            ...
            try {
                ...
                await this.fetchProjects({ page: this.meta.current_page || 1 });
            } catch (error) {
                this.error = error instanceof ApiError ? error.message : 'تعذر حذف المشاريع.';
                redirectIfUnauthorized(error);
                throw error;
            } finally {
                this.saving = false;
        ...
        async uploadImage(uuid, file) {
            ...
            const payload = await api(`/portfolio/${uuid}/images`, {
                method: 'POST',
                body,
            });
        ...
        async reorderImages(uuid, order) {
            const payload = await api(`/portfolio/${uuid}/images/reorder`, {
                method: 'PUT',
                body: { order },
            });
        ...
        async deleteImage(uuid, mediaId) {
            const payload = await api(`/portfolio/${uuid}/images/${mediaId}`, {
                method: 'DELETE',
            });
        ...
        async fetchCategories({ search } = {}) {
            ...
            try {
                ...
                const payload = await api(`/portfolio/categories${qs ? `?${qs}` : ''}`);
        ...
        async createCategory(data) {
            ...
            try {
                const payload = await api('/portfolio/categories', {
                    method: 'POST',
                    body: data,
                });
        ...
        async updateCategory(id, data) {
            ...
            try {
                const payload = await api(`/portfolio/categories/${id}`, {
                    method: 'PUT',
                    body: data,
                });
        ...
        async reorderCategories(order) {
            ...
            try {
                const payload = await api('/portfolio/categories/reorder', {
                    method: 'PUT',
                    body: { order },
                });
        ...
        async fetchSettings({ force = false } = {}) {
            ...
            try {
                const payload = await api('/portfolio/settings');
                this.settings = {
