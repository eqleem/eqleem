...
export const useDigitalServicesStore = defineStore('digitalServicesCatalog', {
    state: () => ({
        type: contentTypeBySlug('digital-services'),
        items: [],
        meta: emptyMeta(),
        search: '',
        loading: false,
    ...
    actions: {
        async fetchDigitalServices({ page = 1, search } = {}) {
            if (search !== undefined) {
                this.search = search;
            }
            ...
            try {
                ...
                const payload = await api(`/digital-services?${params.toString()}`);
        ...
        async goToPage(page) {
            await this.fetchDigitalServices({ page });
        },

        async createDigitalService(title) {
            this.saving = true;
            this.error = null;

            try {
                const payload = await api('/digital-services', {
                    method: 'POST',
                    body: { title },
                });
        ...
        async fetchDigitalService(uuid) {
            this.detailLoading = true;
            this.detailError = null;

            try {
                const payload = await api(`/digital-services/${uuid}`);
                this.detail = payload?.data ?? null;
        ...
        async updateDigitalService(uuid, data) {
            this.saving = true;
        ...
        async deleteDigitalServices(ids) {
            this.saving = true;
            this.error = null;

            try {
                await api('/digital-services', {
                    method: 'DELETE',
                    body: { ids: ids.map(Number) },
                });

                await this.fetchDigitalServices({ page: this.meta.current_page || 1 });
            } catch (error) {
        ...
        async uploadImage(uuid, file) {
            ...
            const payload = await api(`/digital-services/${uuid}/images`, {
                method: 'POST',
                body,
            });
        ...
        async reorderImages(uuid, order) {
            const payload = await api(`/digital-services/${uuid}/images/reorder`, {
                method: 'PUT',
                body: { order },
            });
        ...
        async deleteImage(uuid, mediaId) {
            const payload = await api(`/digital-services/${uuid}/images/${mediaId}`, {
                method: 'DELETE',
            });
        ...
        async fetchCategories({ search, force = false } = {}) {
            ...
            try {
                ...
                const payload = await api(`/digital-services/categories${qs ? `?${qs}` : ''}`);
        ...
        async createCategory(data) {
            ...
            try {
                const payload = await api('/digital-services/categories', {
                    method: 'POST',
                    body: data,
                });
        ...
        async updateCategory(id, data) {
            ...
            try {
                const payload = await api(`/digital-services/categories/${id}`, {
                    method: 'PUT',
                    body: data,
                });
        ...
        async reorderCategories(order) {
            ...
            try {
                const payload = await api('/digital-services/categories/reorder', {
                    method: 'PUT',
                    body: { order },
                });
        ...
        async fetchSettings({ force = false } = {}) {
            ...
            try {
                const payload = await api('/digital-services/settings');
                this.settings = {
