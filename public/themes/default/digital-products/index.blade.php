        ...
        @else
            <div class="grid grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
                @foreach ($products as $product)
                    ...
                    <a
                        href="{{ route('tenant.digital-products.detail', $product->slug) }}"
                        wire:navigate
                        wire:key="digital-product-{{ $product->id }}"
                        class="group rounded-xl md:rounded-2xl overflow-hidden bg-white transition"
                    >
                        <div class="relative">
                            <img src="{{ $imageUrl }}" alt="{{ $product->title }}" class="w-full h-56 md:h-72 object-cover group-hover:scale-105 transition-all duration-500">
                        ...
                        <div class="p-3 border border-neutral-200 border-t-0 rounded-b-xl md:rounded-b-2xl">
                            ...
                            @if ($price > 0)
                                <p class="mt-4 text-xl font-bold font-geist" dir="ltr">{{ money_format($price) }}</p>
                            @endif
                        </div>
                    </a>
                @endforeach
