@php
    $data = $block->data ?? [];
    $showShareButton = (bool) ($data['show_share_button'] ?? true);
    $showThemeToggle = (bool) ($data['show_theme_toggle'] ?? true);
    $showLanguageSwitcher = (bool) ($data['show_language_switcher'] ?? true);
    $showBackButton = (bool) ($data['show_back_button'] ?? true);
    $showClientLogin = (bool) ($data['show_client_login'] ?? true);
    $clientLoginLabel = $data['client_login_label'] ?? 'دخول العملاء';
@endphp
...
<div>
    <nav class="">
        ...
        <div class="flex items-center justify-center w-full h-14">
            @if($showBackButton && !request()->routeIs('tenant.home'))
            <a href="{{route('tenant.home')}}" wire:navigate  id="backBtn" class="h-11 w-11 mt-3 rounded-full bg-black/10 hover:bg-white p-0.5X  flex items-center justify-center transition-all duration-200">
