<header class="relative">
        ...
        @if ($socialLinks->isNotEmpty())
            <div class="flex items-center justify-center gap-x-3 p-2">
                @foreach ($socialLinks as $link)
                    ...
                    >
                        <iconify-icon icon="{{ $link['icon'] }}" class="inline text-2xl" stroke-width="1.5" aria-hidden="true"></iconify-icon>
                    </a>
                @endforeach
            </div>
        @endif
    ...
    <div class="w-full sm:relative Xfixed Xobject-cover min-h-[56px] md:rounded-t-2xl bg-[radial-gradient(ellipse_at_bottom_right,_var(--tw-gradient-stops))] from-primary-600 via-primary-700 to-primary-900 ">
        @php
        ...
        @if ($isCssCover)
            <div
                class="h-52 w-full md:rounded-t-2xl"
        ...
        @elseif ($headerImage)
            <img
                src="{{ $headerImage }}"
                alt="{{ tenant('name') }}"
                class="h-52 w-full object-cover md:rounded-t-2xl opacity-90"
                style="object-position: 50% {{ $headerImagePosition }}%"
            >
        @else
            <img src="{{ asset('assets/images/cover.png') }}" alt="{{ tenant('name') }}" class="h-52 w-full object-cover md:rounded-t-2xl opacity-90">
        @endif
    </div>
