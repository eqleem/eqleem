...
new class extends Component
{
    ...
    {
        return $this->view()
            ->title(tenant()->name . ' - ' . data_get(tenant(), 'data.bio'))
            ->layout('layouts::tenant'); 
    }
};
?>

<x-tenant::layout width="max-w-3xl">
        ...
        <section class="w-full mb-5 flex flex-col gap-y-6">
            ...
            {{-- <x-tenant::block-link title="المتجر" link="{{route('tenant.store.index')}}" icon="hugeicons:store-02" desc="مجموعة مختارة من تشطيبات ديكور المنزل، باركيه وبديل الرخام والخشب والشيبورد. " />
            <x-tenant::block-link title="الدورات" link="{{route('tenant.courses.index')}}" icon="hugeicons:presentation-06" desc="دورات عملية لتعلم التشطيبات والديكور خطوة بخطوة مع تمارين وتطبيقات واقعية." />
            <x-tenant::block-link title="قوائم التشغيل" link="{{route('tenant.playlists.index')}}" icon="hugeicons:playlist-01" desc="قوائم تشغيل فيديو وصوت مع تشغيل متتابع وسريع للمحتوى التعليمي." />
            <x-tenant::block-link title="أعمالنا في التنفيذ" link="{{route('tenant.portfolio.index')}}" icon="hugeicons:folder-library" desc=" معرض مختار لأعمالنا المنفّذة في التشطيبات الداخلية، من الباركيه إلى بدائل الرخام والخشب. " />
            <x-tenant::block-link title="قائمة الطعام" link="{{route('tenant.menu.index')}}" icon="hugeicons:restaurant-01" desc="قائمة وجبات طازجة مع أحجام وإضافات متنوعة، وأسرع طريقة لإضافتها إلى السلة." />
            <x-tenant::block-link title="السيرة الذاتية" link="{{route('tenant.pages.resume')}}" icon="hugeicons:user-account" desc="تعرّف على خبراتي، المهارات، المشاريع المنفذة، وطرق التواصل المباشر في صفحة CV متكاملة." />
            <x-tenant::block-link title="تحديثات المنصة" link="{{route('tenant.pages.changelog')}}" icon="hugeicons:note-01" desc="اطّلع على أحدث الإضافات والتحسينات والإصلاحات في كل إصدار، بتفاصيل واضحة وسريعة." />
            <x-tenant::block-link title="المزايا" link="{{route('tenant.pages.features')}}" icon="hugeicons:magic-wand-02" desc="تعرّف على أهم المزايا التي تجعل تجربة الإدارة والتنفيذ أسرع وأكثر كفاءة." />
            <x-tenant::block-link title="تأجير الوحدات" link="{{route('tenant.properties-rental.index')}}" icon="hugeicons:bed-double" desc="اختر وحدتك المناسبة من الاستديوهات والشقق، حدّد تاريخ الدخول والخروج واحجز مباشرة بسهولة." />
            <x-tenant::block-link title="العقارات" link="{{route('tenant.properties.index')}}" icon="hugeicons:building-03" desc="تصفح عروض التأجير والبيع: شقق، فلل، وأراضٍ مع أسعار واضحة وتواصل مباشر مع المسوق." />
            <x-tenant::block-link title="الباقات والأسعار" link="{{route('tenant.pages.pricing')}}" icon="hugeicons:credit-card-change" desc="اختر الباقة المناسبة لك مع مقارنة واضحة بين المزايا والأسعار." />
            <x-tenant::block-link title="الأسئلة المتكررة" link="{{route('tenant.pages.faq')}}" icon="hugeicons:help-circle" desc="إجابات واضحة وسريعة لأكثر الأسئلة حول الخدمات، الأسعار، وآلية التنفيذ." />
            <x-tenant::block-link title="اتصل بنا" link="{{route('tenant.pages.contact')}}" icon="hugeicons:call" desc="تواصل معنا عبر النموذج، الجوال، البريد، والواتساب مع روابط السوشال." /> --}}
            <x-tenant::block-link title="المدونة" link="{{route('tenant.blog.index')}}" icon="hugeicons:book-open-text" desc=" مقالات ونصائح في التشطيبات والديكور: الباركيه، بديل الرخام، بديل الخشب، والشيبورد. " />
            
            {{-- <x-tenant::block-link title="النشرة البريدية" link="{{route('tenant.newsletter.index')}}" icon="hugeicons:mail-at-sign-02" desc=" اشترك في النشرة البريدية وانظم إلى مجتمع المهتمين في أعمال الفن والديكور. ">
                <div class="">
            ...
            {{-- <x-tenant::block-link title="خدماتنا" link="{{route('tenant.services.index')}}" icon="hugeicons:travel-bag" desc="نقدم خدمات التشطيبات والديكور الداخلي باحترافية، من التصميم حتى التنفيذ بجودة عالية.">
