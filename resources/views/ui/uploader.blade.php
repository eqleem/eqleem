            ...
            @if ($mode === 'profile' && $file)
                ...
                <div class="absolute inset-0">
                    <div class="relative w-full overflow-hidden shadow-inner" style="padding-bottom: 100%">
                        @if(is_string($file))
                            {{-- <img src="{{ \Storage::url($file) }}" class="inset-0 w-full h-full absolute object-cover"> --}}
                            <img src="{{  $file  }}" class="inset-0 w-full h-full absolute object-cover">
                        @elseif(!is_array($file) && is_object($file) && method_exists($file, 'getClientOriginalExtension'))
                            @if(collect(['jpg', 'png', 'jpeg', 'webp'])->contains(strtolower($file->getClientOriginalExtension())))
        ...
        <div>
            @if(is_array($file) && count($file) > 0)
                @foreach($file as $key => $f)
                    @if(!is_array($f) && is_object($f) && method_exists($f, 'getClientOriginalExtension'))
                        <div class="py-3 flex {{ !$loop->last ? 'border-b border-gray-200' : '' }}">
                            <div class="w-16 mr-4 flex-shrink-0 shadow-xs rounded-lg" >
                                @if(collect(['jpg', 'png', 'jpeg', 'webp'])->contains(strtolower($f->getClientOriginalExtension())))
            ...
            @else
                @if($file)
                    <div class="mt-3 flex">
                        <div class="w-16 mr-4 flex-shrink-0 shadow-xs rounded-lg">
                            @if(is_string($file))
                                {{-- Existing file (string URL/path) --}}
                                @php
                                    $extension = strtolower(pathinfo(parse_url($file, PHP_URL_PATH) ?? $file, PATHINFO_EXTENSION));
                                @endphp
                                @if(collect(['jpg', 'png', 'jpeg', 'webp'])->contains($extension))
                                
                                    <div class="relative pb-16 w-full overflow-hidden rounded-lg xborder border-gray-100">
                        ...
                        <div class="px-2">
                            ...
                            <div class="text-xs w-36 truncate">
                                @if(is_string($file))
                                    <span class="truncate text-xs text-overflow-ellipsis">{{ basename(parse_url($file, PHP_URL_PATH) ?? $file) }}</span>
                                @elseif(!is_array($file) && is_object($file) && method_exists($file, 'getClientOriginalName'))
                                    <span class="truncate text-xs text-overflow-ellipsis">{{ $file->getClientOriginalName() }}</span>
                                @else
                                    File
                                @endif
                            </div>
                            ...
                            @if($file)
                                @if(!is_array($file) && is_object($file) && method_exists($file, 'getFilename'))
                                    {{-- TemporaryUploadedFile - use removeUpload --}}
                                    <button
                                        wire:loading.attr="disabled" 
                                        type="button" 
                                        wire:confirm="why"
                                        x-on:click.prevent="$wire.removeUpload('{{ $attributes->wire('model')->value }}', '{{ $file->getFilename() }}')" 
                                        class="text-xs text-red-500 appearance-none hover:underline cursor-pointer">
                                         حذف
                                    </button>
                                @elseif(is_string($file))
                                    {{-- Existing file (string) - clear the property directly --}}
                                    <button
                                        wire:loading.attr="disabled" 
                                        type="button" 
                                        wire:confirm="why"
                                        x-on:click.prevent="$wire.set('{{ $attributes->wire('model')->value }}', null)" 
                                        class="text-xs text-red-500 appearance-none hover:underline cursor-pointer">
                                         حذف
                                    </button>
                                @endif
