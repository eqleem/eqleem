<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <ui:modal :title="$addingParentId ? 'إضافة تصنيف فرعي' : 'إضافة تصنيف'" size="lg" name="add-unit-category">
            <livewire:admin::page.content.unit-rental.category-form
                :contentType="$contentType"
                :default-parent-id="$addingParentId"
                :key="'add-unit-category-'.($addingParentId ?? 'root')"
            />
        </ui:modal>
    </div>
    ...
    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($categories->isEmpty())
            <ui:empty subtitle="سيتم عرض تصنيفات الوحدات هنا بعد إضافتها.">
                لا توجد تصنيفات.
                <x-slot:icon>
        ...
        @else
            ...
            >
                @foreach ($categories as $category)
                    <li
                        wire:key="unit-category-{{ $category->id }}"
                        @if ($search === '')
...
<button type="button"
    ...
    <ui:modal title="تعديل التصنيف" size="lg" name="edit-unit-category">
        @if ($editingCategoryId)
            <livewire:admin::page.content.unit-rental.category-form
                :contentType="$contentType"
                :category-id="$editingCategoryId"
                :key="'edit-unit-category-'.$editingCategoryId"
            />
        @endif
    </ui:modal>
</div>
...
new class extends \Livewire\Component
{
    ...
    {
        if ($parentId !== null && ! Taxonomy::query()->type('unit_category')->whereKey($parentId)->exists()) {
            return;
        }

        $this->addingParentId = $parentId;
        $this->dispatch('openmodal', modal: 'add-unit-category');
    }
    ...
    {
        if (! Taxonomy::query()->type('unit_category')->whereKey($categoryId)->exists()) {
            return;
        }

        $this->editingCategoryId = $categoryId;
        $this->dispatch('openmodal', modal: 'edit-unit-category');
    }

    public function delete(int $id): void
    {
        Taxonomy::query()->type('unit_category')->whereKey($id)->first()?->delete();
    ...
    {
        $categories = Taxonomy::flatTree('unit_category');
