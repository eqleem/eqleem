<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <div>
            <ui:button @click.prevent="$dispatch('openmodal', { modal: 'add-course' })" label="دورة جديدة"
                icon="square-rounded-plus" />
        </div>

        <ui:modal title="إضافة دورة تدريبية جديدة" size="2xl" name="add-course">
            <livewire:admin::page.content.courses.add-course :contentType="$contentType" />
        </ui:modal>
    </div>

    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($results->count() === 0)
            <ui:empty subtitle="سيتم عرض الدورات التدريبية هنا بعد إضافتها.">
                    ...
                    <div wire:key="course-{{ $item->uuid }}"
                        ...
                        <div class="py-3 w-full">
                            <a href="{{ route('admin.page.home', ['tab' => $contentType['tab_id'], 'item' => $item->uuid]) }}"
                                ...
                                @php($imageUrl = $item->getFirstMediaUrl('course-media') ?: $item->avatar)
                                <img
                                    ...
                                    <div class="flex items-center gap-x-2 mt-1">
                                        ...
                                        <p class="mt-1 flex text-xs items-center gap-1 text-gray-500">
                                            ...
                                            <span class="inline-block text-gray-500">
                                                {{ $item->courseLevelLabel() }}
                                            </span>
                                            @php($hours = (int) data_get($item->data, 'hours', 0))
                                            @if ($hours > 0)
                                                <span class="inline-block text-gray-500">
                                                    {{ $hours }} ساعة
                                                </span>
                                            @endif
                                            @php($lessonsCount = $item->courseLessonCount())
                                            @if ($lessonsCount > 0)
                                                <span class="inline-block text-gray-500">
