<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <div>
            <ui:button @click.prevent="$dispatch('openmodal', { modal: 'add-store-product' })" label="منتج جديد"
                icon="square-rounded-plus" />
        </div>

        <ui:modal title="إضافة منتج جديد" size="2xl" name="add-store-product">
            <livewire:admin::page.content.store.add-product :contentType="$contentType" />
        </ui:modal>
    </div>

    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($results->count() === 0)
            <ui:empty subtitle="سيتم عرض المنتجات هنا بعد إضافتها.">
                لا توجد منتجات.
                <x-slot:icon>
        ...
        @else
            <div class="">
                @foreach ($results as $item)
                    <div wire:key="store-product-{{ $item->uuid }}"
                        ...
                        <div class="py-3 w-full">
                            <a href="{{ route('admin.page.home', ['tab' => $contentType['tab_id'], 'item' => $item->uuid]) }}"
                                ...
                                @php($imageUrl = $item->getFirstMediaUrl('store-media') ?: $item->avatar)
                                <img
