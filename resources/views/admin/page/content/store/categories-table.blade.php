<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <ui:modal :title="$addingParentId ? 'إضافة قسم فرعي' : 'إضافة تصنيف'" size="lg" name="add-store-category">
            <livewire:admin::page.content.store.category-form
                :contentType="$contentType"
                :default-parent-id="$addingParentId"
                :key="'add-store-category-'.($addingParentId ?? 'root')"
            />
        </ui:modal>
    </div>
    ...
    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($categories->isEmpty())
            <ui:empty subtitle="سيتم عرض تصنيفات المتجر هنا بعد إضافتها.">
                لا توجد تصنيفات.
                <x-slot:icon>
        ...
        @else
            ...
            >
                @foreach ($categories as $category)
                    <li
                        wire:key="store-category-{{ $category->id }}"
                        @if ($search === '')
    ...
    <ui:modal title="تعديل التصنيف" size="lg" name="edit-store-category">
        @if ($editingCategoryId)
            <livewire:admin::page.content.store.category-form
                :contentType="$contentType"
                :category-id="$editingCategoryId"
                :key="'edit-store-category-'.$editingCategoryId"
            />
        @endif
    </ui:modal>
</div>
...
new class extends \Livewire\Component
{
    ...
    {
        if ($parentId !== null && ! Taxonomy::query()->type('store_category')->whereKey($parentId)->exists()) {
            return;
        }

        $this->addingParentId = $parentId;
        $this->dispatch('openmodal', modal: 'add-store-category');
    }

    public function openEditModal(int $categoryId): void
    {
        if (! Taxonomy::query()->type('store_category')->whereKey($categoryId)->exists()) {
            return;
        }

        $this->editingCategoryId = $categoryId;
        $this->dispatch('openmodal', modal: 'edit-store-category');
    }

    public function delete(int $id): void
    {
        Taxonomy::query()->type('store_category')->whereKey($id)->first()?->delete();
    ...
    {
        $categories = Taxonomy::flatTree('store_category');
