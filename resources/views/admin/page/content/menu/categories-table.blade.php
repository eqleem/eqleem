<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <ui:modal :title="$addingParentId ? 'إضافة قسم فرعي' : 'إضافة تصنيف'" size="lg" name="add-menu-category">
            <livewire:admin::page.content.menu.category-form
                :contentType="$contentType"
                :default-parent-id="$addingParentId"
                :key="'add-menu-category-'.($addingParentId ?? 'root')"
            />
        </ui:modal>
    </div>
    ...
    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($categories->isEmpty())
            <ui:empty subtitle="سيتم عرض تصنيفات القائمة هنا بعد إضافتها.">
                لا توجد تصنيفات.
                <x-slot:icon>
    ...
    <ui:modal title="تعديل التصنيف" size="lg" name="edit-menu-category">
        @if ($editingCategoryId)
            <livewire:admin::page.content.menu.category-form
                :contentType="$contentType"
                :category-id="$editingCategoryId"
                :key="'edit-menu-category-'.$editingCategoryId"
            />
        @endif
    </ui:modal>
</div>
...
new class extends \Livewire\Component
{
    ...
    {
        if ($parentId !== null && ! Taxonomy::query()->type('menu_category')->whereKey($parentId)->exists()) {
            return;
        }

        $this->addingParentId = $parentId;
        $this->dispatch('openmodal', modal: 'add-menu-category');
    }

    public function openEditModal(int $categoryId): void
    {
        if (! Taxonomy::query()->type('menu_category')->whereKey($categoryId)->exists()) {
            return;
        }

        $this->editingCategoryId = $categoryId;
        $this->dispatch('openmodal', modal: 'edit-menu-category');
    }

    public function delete(int $id): void
    {
        Taxonomy::query()->type('menu_category')->whereKey($id)->first()?->delete();
    ...
    {
        $categories = Taxonomy::flatTree('menu_category');
