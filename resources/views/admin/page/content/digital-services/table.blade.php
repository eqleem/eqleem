<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <div>
            <ui:button @click.prevent="$dispatch('openmodal', { modal: 'add-digital-service' })" label="خدمة جديدة"
                icon="square-rounded-plus" />
        </div>

        <ui:modal title="إضافة خدمة رقمية جديدة" size="2xl" name="add-digital-service">
            <livewire:admin::page.content.digital-services.add-service :contentType="$contentType" />
        </ui:modal>
    </div>

    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($results->count() === 0)
            <ui:empty subtitle="سيتم عرض الخدمات الرقمية هنا بعد إضافتها.">
                لا توجد خدمات رقمية.
                <x-slot:icon>
                    ...
                    <div wire:key="digital-service-{{ $item->uuid }}"
                        ...
                        <div class="py-3 w-full">
                            <a href="{{ route('admin.page.home', ['tab' => $contentType['tab_id'], 'item' => $item->uuid]) }}"
                                ...
                                @php($imageUrl = $item->getFirstMediaUrl('digital-service-media') ?: $item->avatar)
                                <img
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-x-2 mt-1">
                                        ...
                                        <p class="mt-1 flex text-xs items-center gap-1 text-gray-500">
                                            ...
                                            @endif
                                            @php($deliveryDays = data_get($item->data, 'delivery_days'))
                                            @if ($deliveryDays)
                                                <span class="inline-block text-gray-500">
                                                    {{ $deliveryDays }} {{ (int) $deliveryDays === 1 ? 'يوم' : 'أيام' }}
                                                </span>
                                            @endif
                                        </p>
                                    </div>
