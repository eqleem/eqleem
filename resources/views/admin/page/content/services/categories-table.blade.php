<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <div>
            <ui:button wire:click="openAddModal" label="تصنيف جديد" icon="square-rounded-plus" />
        </div>

        <ui:modal :title="$addingParentId ? 'إضافة قسم فرعي' : 'إضافة تصنيف'" size="lg" name="add-service-category">
            <livewire:admin::page.content.services.category-form
                :contentType="$contentType"
                :default-parent-id="$addingParentId"
                :key="'add-service-category-'.($addingParentId ?? 'root')"
            />
        </ui:modal>
    </div>
    ...
    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($categories->isEmpty())
            <ui:empty subtitle="سيتم عرض تصنيفات الخدمات هنا بعد إضافتها.">
                لا توجد تصنيفات.
                <x-slot:icon>
        ...
        @else
            ...
            >
                @foreach ($categories as $category)
                    <li
                        wire:key="service-category-{{ $category->id }}"
                        @if ($search === '')
    ...
    <ui:modal title="تعديل التصنيف" size="lg" name="edit-service-category">
        @if ($editingCategoryId)
            <livewire:admin::page.content.services.category-form
                :contentType="$contentType"
                :category-id="$editingCategoryId"
                :key="'edit-service-category-'.$editingCategoryId"
            />
        @endif
    </ui:modal>
</div>
...
new class extends \Livewire\Component
{
    ...
    {
        if ($parentId !== null && ! Taxonomy::query()->type('service_category')->whereKey($parentId)->exists()) {
            return;
        }

        $this->addingParentId = $parentId;
        $this->dispatch('openmodal', modal: 'add-service-category');
    }
    ...
    {
        if (! Taxonomy::query()->type('service_category')->whereKey($categoryId)->exists()) {
            return;
        }

        $this->editingCategoryId = $categoryId;
        $this->dispatch('openmodal', modal: 'edit-service-category');
    }

    public function delete(int $id): void
    {
        Taxonomy::query()->type('service_category')->whereKey($id)->first()?->delete();
    ...
    {
        $categories = Taxonomy::flatTree('service_category');
