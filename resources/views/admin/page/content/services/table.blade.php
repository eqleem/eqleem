<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <div>
            <ui:button @click.prevent="$dispatch('openmodal', { modal: 'add-service' })" label="خدمة جديدة"
                icon="square-rounded-plus" />
        </div>

        <ui:modal title="إضافة خدمة جديدة" size="2xl" name="add-service">
            <livewire:admin::page.content.services.add-service :contentType="$contentType" />
        </ui:modal>
    </div>

    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($results->count() === 0)
            <ui:empty subtitle="سيتم عرض الخدمات هنا بعد إضافتها.">
                لا توجد خدمات.
                <x-slot:icon>
        ...
        @else
            <div class="">
                @foreach ($results as $item)
                    <div wire:key="service-{{ $item->uuid }}"
                        ...
                        <div class="py-3 w-full">
                            <a href="{{ route('admin.page.home', ['tab' => $contentType['tab_id'], 'item' => $item->uuid]) }}"
                                ...
                                @php($imageUrl = $item->getFirstMediaUrl('service-media') ?: $item->avatar)
                                <img
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-x-2 mt-1">
                                        ...
                                        <p class="mt-1 flex text-xs items-center gap-1 text-gray-500">
                                            ...
                                            @endif
                                            @if (data_get($item->data, 'duration_minutes'))
                                                <span class="inline-block text-gray-500">
                                                    {{ data_get($item->data, 'duration_minutes') }} د
                                                </span>
                                            @endif
                                        </p>
                                    </div>
...
new class extends Livewire\Component
{
    ...
    public function clone(int $id): void
    {
        ...
        $clone->syncTaxonomiesOfType(
            'service_category',
            $original->taxonomiesOfType('service_category')->pluck('id')->all(),
        );
        ...
        $this->dispatch('notify', text: 'تم نسخ الخدمة.');
        $this->dispatch(
            'openContentItem',
