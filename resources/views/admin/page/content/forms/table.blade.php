<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <div class=" ">
            <ui:button @click.prevent="$dispatch('openmodal', { modal: 'add-form' })" label="نموذج جديد"
                icon="square-rounded-plus" />
        </div>

        <ui:modal title="إضافة نموذج جديد" size="2xl" name="add-form">
            <livewire:admin::page.content.forms.add-form :contentType="$contentType" />
        </ui:modal>
    </div>

    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($results->count() === 0)
            <ui:empty subtitle="سيتم عرض النماذج هنا بعد إضافتها.">
                لا توجد نماذج.
                <x-slot:icon>
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-x-2 mt-1">
                                        ...
                                        <p class="mt-1 flex text-xs items-center gap-1 text-gray-500">
                                            ...
                                            <span
                                                class="truncate inline-flex items-center gap-x-1 bg-gray-100 p-1 px-2 rounded-md text-xs">
                                                {{ \Illuminate\Support\Number::format($item->form_submissions_count) }}
                                                {{ $item->form_submissions_count === 1 ? 'إرسالية' : 'إرساليات' }}
                                            </span>
                                            @if ($item->updated_at)
...
new class extends Livewire\Component
{
    ...
    public function clone(int $id): void
    {
        ...
        $this->dispatch('notify', text: 'تم نسخ النموذج.');
        $this->dispatch(
            'openContentItem',
