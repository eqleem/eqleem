<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <ui:modal :title="$addingParentId ? 'إضافة قسم فرعي' : 'إضافة تصنيف'" size="lg" name="add-digital-product-category">
            <livewire:admin::page.content.digital-products.category-form
                :contentType="$contentType"
                :default-parent-id="$addingParentId"
                :key="'add-digital-product-category-'.($addingParentId ?? 'root')"
            />
        </ui:modal>
    </div>
    ...
    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($categories->isEmpty())
            <ui:empty subtitle="سيتم عرض تصنيفات المنتجات الرقمية هنا بعد إضافتها.">
                لا توجد تصنيفات.
                <x-slot:icon>
        ...
        @else
            ...
            >
                @foreach ($categories as $category)
                    <li
                        wire:key="digital-product-category-{{ $category->id }}"
                        @if ($search === '')
...
<button type="button"
    ...
    <ui:modal title="تعديل التصنيف" size="lg" name="edit-digital-product-category">
        @if ($editingCategoryId)
            <livewire:admin::page.content.digital-products.category-form
                :contentType="$contentType"
                :category-id="$editingCategoryId"
                :key="'edit-digital-product-category-'.$editingCategoryId"
            />
        @endif
    </ui:modal>
</div>
...
new class extends \Livewire\Component
{
    ...
    #[On('updateDigitalProductCategoryList')]
    ...
    {
        if ($parentId !== null && ! Taxonomy::query()->type('digital_store_category')->whereKey($parentId)->exists()) {
            return;
        }

        $this->addingParentId = $parentId;
        $this->dispatch('openmodal', modal: 'add-digital-product-category');
    }

    public function openEditModal(int $categoryId): void
    {
        if (! Taxonomy::query()->type('digital_store_category')->whereKey($categoryId)->exists()) {
            return;
        }

        $this->editingCategoryId = $categoryId;
        $this->dispatch('openmodal', modal: 'edit-digital-product-category');
    }

    public function delete(int $id): void
    {
        Taxonomy::query()->type('digital_store_category')->whereKey($id)->first()?->delete();
    ...
    {
        $categories = Taxonomy::flatTree('digital_store_category');
