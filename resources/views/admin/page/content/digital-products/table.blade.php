<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <div>
            <ui:button @click.prevent="$dispatch('openmodal', { modal: 'add-digital-product' })" label="منتج جديد"
                icon="square-rounded-plus" />
        </div>

        <ui:modal title="إضافة منتج رقمي جديد" size="2xl" name="add-digital-product">
            <livewire:admin::page.content.digital-products.add-product :contentType="$contentType" />
        </ui:modal>
    </div>

    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($results->count() === 0)
            <ui:empty subtitle="سيتم عرض المنتجات الرقمية هنا بعد إضافتها.">
        ...
        @else
            <div class="">
                @foreach ($results as $item)
                    <div wire:key="digital-product-{{ $item->uuid }}"
                        ...
                        <div class="py-3 w-full">
                            <a href="{{ route('admin.page.home', ['tab' => $contentType['tab_id'], 'item' => $item->uuid]) }}"
                                ...
                                @php($imageUrl = $item->getFirstMediaUrl('digital-product-media') ?: $item->avatar)
                                <img
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-x-2 mt-1">
                                        ...
                                        <p class="mt-1 flex text-xs items-center gap-1 text-gray-500">
                                            ...
                                            @endif
                                            @php($downloadsCount = $item->getMedia('digital-product-downloads')->count())
                                            @if ($downloadsCount > 0)
                                                <span class="inline-block text-gray-500">
                                                    {{ $downloadsCount }} {{ $downloadsCount === 1 ? 'ملف' : 'ملفات' }}
                                                </span>
                                            @endif
                                        </p>
                                    </div>
