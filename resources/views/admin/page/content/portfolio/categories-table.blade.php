<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <ui:modal :title="$addingParentId ? 'إضافة قسم فرعي' : 'إضافة تصنيف'" size="lg" name="add-portfolio-category">
            <livewire:admin::page.content.portfolio.category-form
                :contentType="$contentType"
                :default-parent-id="$addingParentId"
                :key="'add-portfolio-category-'.($addingParentId ?? 'root')"
            />
        </ui:modal>
    </div>
    ...
    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($categories->isEmpty())
            <ui:empty subtitle="سيتم عرض تصنيفات الأعمال هنا بعد إضافتها.">
                لا توجد تصنيفات.
                <x-slot:icon>
        ...
        @else
            ...
            >
                @foreach ($categories as $category)
                    <li
                        wire:key="portfolio-category-{{ $category->id }}"
                        @if ($search === '')
...
<button type="button"
    ...
    <ui:modal title="تعديل التصنيف" size="lg" name="edit-portfolio-category">
        @if ($editingCategoryId)
            <livewire:admin::page.content.portfolio.category-form
                :contentType="$contentType"
                :category-id="$editingCategoryId"
                :key="'edit-portfolio-category-'.$editingCategoryId"
            />
        @endif
    </ui:modal>
</div>
...
new class extends \Livewire\Component
{
    ...
    #[On('updatePortfolioCategoryList')]
    ...
    {
        if ($parentId !== null && ! Taxonomy::query()->type('portfolio_category')->whereKey($parentId)->exists()) {
            return;
        }

        $this->addingParentId = $parentId;
        $this->dispatch('openmodal', modal: 'add-portfolio-category');
    }

    public function openEditModal(int $categoryId): void
    {
        if (! Taxonomy::query()->type('portfolio_category')->whereKey($categoryId)->exists()) {
            return;
        }

        $this->editingCategoryId = $categoryId;
        $this->dispatch('openmodal', modal: 'edit-portfolio-category');
    }

    public function delete(int $id): void
    {
        Taxonomy::query()->type('portfolio_category')->whereKey($id)->first()?->delete();
