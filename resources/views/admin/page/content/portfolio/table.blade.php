<div class="divide-y divide-gray-200 divide-dotted">
    <div class="bg-gray-100 p-3 flex items-center gap-x-4 w-full">
        ...
        <div>
            <ui:button @click.prevent="$dispatch('openmodal', { modal: 'add-portfolio-project' })" label="مشروع جديد"
                icon="square-rounded-plus" />
        </div>

        <ui:modal title="إضافة مشروع جديد" size="2xl" name="add-portfolio-project">
            <livewire:admin::page.content.portfolio.add-project :contentType="$contentType" />
        </ui:modal>
    </div>

    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        @if ($results->count() === 0)
            <ui:empty subtitle="سيتم عرض المشاريع هنا بعد إضافتها.">
                لا توجد مشاريع.
                <x-slot:icon>
        ...
        @else
            <div class="pb-4">
                @foreach ($results as $item)
                    <div wire:key="portfolio-project-{{ $item->uuid }}"
                        ...
                        <div class="py-3 w-full">
                            <a href="{{ route('admin.page.home', ['tab' => $contentType['tab_id'], 'item' => $item->uuid]) }}"
                                ...
                                @php($imageUrl = $item->getFirstMediaUrl('portfolio-media') ?: contentImageUrl(data_get($item->data, 'images.0')) ?: contentImageUrl(data_get($item->data, 'image')) ?: $item->avatar)
                                <img
...
new class extends Livewire\Component
{
    ...
    {
        Content::query()->type('portfolio')->whereKey($id)->first()?->delete();
