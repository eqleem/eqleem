<div class="divide-y divide-gray-200 divide-dotted">
    ...
    <div class="relative last-child:rounded-b-2xl pb-4X p-1">
        ...
        @if ($results->count() === 0)
            <ui:empty subtitle="ستظهر فواتير هذا العميل هنا بعد تسجيل أي دفعة أو إصدار فاتورة لطلباته.">
                لا توجد فواتير.
                <x-slot:icon>
...
new class extends Livewire\Component {
    ...
    protected function baseQuery()
    {
        $query = Invoice::query()
            ->with(['invoicable'])
            ->where('invoicable_type', Order::class)
            ->whereHasMorph('invoicable', [Order::class], function ($query): void {
        ...
        return $query->where(function ($query) use ($search): void {
            $query->where('number', 'like', $search)
                ...
                ->orWhere(function ($query) use ($search): void {
                    $query->whereHasMorph('invoicable', [Order::class], function ($query) use ($search): void {
                        $query->where('number', 'like', $search)
